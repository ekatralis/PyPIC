import numpy as np
from . import CUDAFiniteDifferences_Staircase_SquareGrid as PIC_FD
from . import CUDAFiniteDifferences_ShortleyWeller_SquareGrid as PIC_FDSW
...
class AddInternalGrid(PyPIC_Scatter_Gather):
    def __init__(self, pic_external, x_min_internal, x_max_internal, y_min_internal, y_max_internal, Dh_internal, N_nodes_discard,
                sparse_solver = 'cuDSS', include_solver = True):
        ...
        self.x_min_internal = cp.array(x_min_internal)
        self.x_max_internal = cp.array(x_max_internal)
        self.y_min_internal = cp.array(y_min_internal)
        self.y_max_internal = cp.array(y_max_internal)
        self.Dh_internal = Dh_internal
        self.N_nodes_discard = N_nodes_discard
        self.D_discard = cp.array(N_nodes_discard*Dh_internal)
    ...
    def gather(self, x_mp, y_mp):
        mask_internal = cp.logical_and(\
            cp.logical_and(x_mp > self.x_min_internal + self.D_discard, 
                           x_mp < self.x_max_internal - self.D_discard),
            cp.logical_and(y_mp > self.y_min_internal + self.D_discard, 
                           y_mp < self.y_max_internal - self.D_discard))
                           
        idx_internal = cp.flatnonzero(mask_internal)
        idx_external = cp.flatnonzero(~mask_internal)
    ...
    def get_state_object(self):
        ...
        state = AddInternalGrid(state_external, self.x_min_internal.get(), self.x_max_internal.get(), self.y_min_internal.get(), 
                self.y_max_internal.get(), self.Dh_internal, self.N_nodes_discard, sparse_solver=self.sparse_solver, include_solver = False)
